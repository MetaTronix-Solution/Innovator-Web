...
interface UserDropdownProps {
  ...
  onLogout: () => void;
  getProfileImage: () => string | null;
  onClose: () => void; // 1. Added onClose prop from parent component to handle closing state toggle
}
...
const UserDropdown = ({
  ...
  // 2. Automatically close dropdown when the window or layout scrolls
  useEffect(() => {
    const handleScroll = () => {
      onClose();
    };

    // 'true' uses event capturing to catch scrolls anywhere on the page/layout
  ...
  return (
    <div className="absolute top-12 right-0 w-[360px] bg-card border border-border shadow-2xl rounded-xl overflow-hidden z-[60] p-4 animate-in fade-in zoom-in-95 duration-200">
      <div className="p-2 mb-2 rounded-lg shadow-md border border-border bg-background/50">
        {/* 3. Replaced dynamic router.push click handler setup with optimized Link component overlay */}
        <Link
          href={`/${userId}`}
          onClick={onClose} // Closes menu panel frame on navigate execution
          className="flex items-center gap-3 p-2 hover:bg-accent rounded-md transition-colors cursor-pointer"
        >
          <div className="relative w-10 h-10 rounded-full overflow-hidden border border-border flex items-center justify-center bg-muted shrink-0">
            {profileImage ? (
              <Image
                src={profileImage}
                alt="User"
