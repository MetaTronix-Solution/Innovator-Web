  ...
  useEffect(() => {
    ...
    const fetchInitialNotifications = async () => {
      try {
        ...
        const filtered = data.filter((n: any) => n.type !== "chat_message");
        setInitialNotifications(filtered);
        setUnreadCount(
          filtered.filter(
            (n: any) => !n.is_read && n.notification_type !== "chat_message",
          ).length,
        );
      } catch (err) {
    ...
    <>
      ...
      <nav
        className={`sticky top-0 z-50 w-full border-b transition-all duration-300 ${
          scrolled
            ? "backdrop-blur-3xl border-white/10 dark:border-white/10"
      ...
      >
        ...
        >
          <div className="flex items-center cursor-pointer flex-1 gap-2">
            <Link
              href="/"
              className="shrink-0 hidden md:flex items-center gap-1"
              onClick={(e) => {
                if (pathname === "/") {
                  e.preventDefault();
                  smoothScrollToTop();
            ...
            <button
              className="shrink-0 flex md:hidden items-center gap-1 focus:outline-none"
