...
//     // 1. Check if Category is selected (Mandatory)
//     if (!selectedCategoryId) {
//       return toast.error("Please select a category before posting.");
//     }

//     // 2. Content/Media Check
...
//     // 3. Append the single category ID
//     // Note: Most Django backends expect the field name 'categories'
//     // even for a single ID if it's a ManyToMany relationship.
...
//         setMediaFiles([]);
//         setPreviews([]);
//         setSelectedCategoryId(null); // Reset single selection
//         onClose();
//       } else {
...
export default function CreatePostModal({
  ...
  const handlePostSubmit = async () => {
    ...
    // 4. Append media files - backend will handle them as array
    mediaFiles.forEach((file) => {
      formData.append("media", file);
    });
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Log the new post structure to debug
