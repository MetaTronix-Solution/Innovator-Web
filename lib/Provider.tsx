...
function AuthHydrator({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const rehydrate = async () => {
      // 🚀 CRITICAL FIX: Check if a manual form login is running
      // If a form login just happened, skip this cycle to avoid overwriting state with a 401
      if (
        typeof window !== "undefined" &&
        sessionStorage.getItem("just_logged_in") === "true"
      ) {
        sessionStorage.removeItem("just_logged_in");
        hydratedRef.current = true; // Block subsequent re-runs for this mount cycle
        return;
      }
